...
## diseasemap.dt[Condition %in% c("Diabetes_without_complications","Peptic_ulcer_disease")]
...
summary.gender.dt <- out.disease.patient[!(Gender %in% c("MISSING", "")),.(count=as.numeric(.(.N))),by=c("Condition","Gender")]

#Then I get count information for All patients combined, by condition
summary.all.dt <- out.disease.patient[!(Gender %in% c("MISSING", "")),.(Gender="all",count=as.numeric(.(.N))),by=Condition]
...
summary.pct.dt <- summary.concat.dt[,list(Gender, pct=2*100*count/sum(count)),by=Condition]
...
```
Observing the disease groups, and the percentage of either Men or Women experiencing the respective diseases, we see a few patterns. There is generally a higher percentage of women for each disease group, with a few exceptions, for example Congestive Heart Failure, where men are 51.8% and women are 48.1%. This could be attributed to the difference in number of men and women in the data.
...
```{r}
#using a for loop to get all mortality rates from 2008-2005
